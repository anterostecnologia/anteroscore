...
public class JEncoder extends Frame {
    ...
    public void compressImage() {
		...
		// Initialisation de l'op�ration de compression,DCT & la quatification:
        ...
        for (i = 0; i < width / BlocSize; i++) {
            for (j = 0; j < height / BlocSize; j++) {
                ...
                xpos = i * BlocSize;
                ypos = j * BlocSize;
				// D�composition de la matrice image "outputArray" en des blocs 8x8
                ...
                // Reconstruct the compressed data in an image... (Reconstruction de l'image du donn�es compr�ss�es)
                for (a = 0; a < BlocSize; a++) {
                    for (b = 0; b < BlocSize; b++) {
       ...
       /*
		...
		// la m�thode compressImage de l'objet dctTrans prend comme param�tre un tableau 1D, et une variable bool�enne et nous donne comme variable de sortie un tableau 1D d'entier:
        compressed = CompressionTools.compressImage(one, width, height);

        // Run the decompressor (Faire fonctionner le d�compresseur)
        int decompressed[] = new int[width * height];
		// Appel � la m�thode decompressImage de l'objet dctTrans : cette m�thode prend comme param�tres le tableau 1D (tableau de donn�es compr�ss�s "compressed" cr�er par la m�thode compressImage) et une variable bool�enne:
        decompressed = CompressionTools.decompressImage(compressed, width, height);
		// Test sur l'op�ration de d�compr�ssion s'il y a des erreurs on affiche message
        if (one != decompressed) {
            System.out.println("Compression error");
        }
		// Conversion de la tableau 1D de donn�es compr�sse�s � un tableau 2D (image reconstruite):
		...
		// Initialisation de l'op�ration de d�compression, d�quatification & la DCT inverse:
        ...
        for (i = 0; i < width/BlocSize; i++) {
            for (j = 0; j < height/BlocSize; j++) {
				...
				// Faire fonctionner l'op�ration de d�quantification de l'objet dctTrans, qui prend comme param�tre un tableru 2D (dctArray2) et une variable boole�nne
                dctArray3 = CompressionTools.dequantitizeImage(dctArray2, quantum, BlocSize);

                // Run the inverse DCT
				// Faire fonctionner l'op�ration de la DCT inverse:m�thode de l'objet dctTrans, prend comme parm�tre un tableau 2D (dctArray3), retourne un tab 2D:
				// Cr�ation du tab dctArray4:
		...
		// Appel a la fonction "makeImage" qui prend comme param�tre un tableau 2D (reconstImage):
	...
	// M�thode "makeImage" qui se charge pour traiter un tableau image et l'affiche dans une fenetre(l'objet ImageWindow):

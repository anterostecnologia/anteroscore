...
/**
 * A hash table with <em>weak keys</em>, full concurrency of retrievals, and adjustable expected concurrency for
 * updates. Similar to {@link java.util.WeakHashMap}, entries of this table are periodically removed once their
 * corresponding keys are no longer referenced outside of this table. In other words, this table will not prevent a key
 * from being discarded by the garbage collector. Once a key has been discarded by the collector, the corresponding
 * entry is no longer visible to this table; however, the entry may occupy space until a future table operation decides
 * to reclaim it. For this reason, summary functions such as <tt>size</tt> and <tt>isEmpty</tt> might return a value
 * greater than the observed number of entries. In order to support a high level of concurrency, stale entries are only
 * reclaimed during blocking (usually mutating) operations.
 *
 * While keys in this table are only held using a weak reference, values are held using a normal strong reference. This
 * provides the guarantee that a value will always have at least the same life-span as it's key. For this reason, care
 * should be taken to ensure that a value never refers, either directly or indirectly, to its key, thereby preventing
 * reclamation. If weak values are desired, one can simply use a {@link WeakReference} for the value type.
 *
 * Just like {@link java.util.ConcurrentHashMap}, this class obeys the same functional specification as
 * {@link java.util.Hashtable}, and includes versions of methods corresponding to each method of <tt>Hashtable</tt>.
 * However, even though all operations are thread-safe, retrieval operations do <em>not</em> entail locking, and there
 * is <em>not</em> any support for locking the entire table in a way that prevents all access. This class is fully
 * interoperable with <tt>Hashtable</tt> in programs that rely on its thread safety but not on its synchronization
 * details.
 *
 * <p>
...
public class ConcurrentWeakHashMap<K, V> extends AbstractMap<K, V> implements java.util.concurrent.ConcurrentMap<K, V>, Serializable {
	 ...
	 * is critical because ConcurrentWeakHashMap uses power-of-two length hash tables, that otherwise encounter
	...
	static final class Segment<K, V> extends ReentrantLock implements Serializable {
		 ...
		 * 
		 * Read operations can thus proceed without locking, but rely on selected uses of volatiles to ensure that
		 * completed write operations performed by other threads are noticed. For most purposes, the "count" field,
		 * tracking the number of elements, serves as that volatile variable ensuring visibility. This is convenient
		 * because this field needs to be read in many read operations anyway:
		 * 
		 * - All (unsynchronized) read operations must first read the "count" field, and should not look at table
		 * entries if it is 0.
		 * 
		 ...
		 * inconsistent data. This is made easier by the nature of the read operations in Map. For example, no operation
		 * can reveal that the table has grown but the threshold has not yet been updated, so there are no atomicity
	 ...
	 *            the estimated number of concurrently updating threads. The implementation performs internal sizing to
	...
	/**
	 * Save the state of the <tt>ConcurrentWeakHashMap</tt> instance to a stream (i.e., serialize it).
	 * 
	 * @param s
	 *            the stream
	...
	/**
	 * Reconstitute the <tt>ConcurrentWeakHashMap</tt> instance from a stream (i.e., deserialize it).
	 * 
	 * @param s
	 *            the stream
	 */
